...
public abstract class AbstractPlaceDetailsConverter implements PlaceDetailsConverter {
    ...
    public Address convert(PlaceDetails place, Map<String, AddressComponent> components) {
        String placeName = place.getDisplayName() != null ? place.getDisplayName().getText() : "";
        ...
        com.haulmont.shamrock.as.dto.Location location = GoogleAddressUtils.convert(place.getLocation());
        ...
        if (elements != null && elements.length != 0) {
            ...
            for (AddressComponent addressComponent : components.values()) {
                if (addressComponent.getTypes().containsAll(sElems))
                    return StringUtils.trim(shortName ? addressComponent.getShortText() : addressComponent.getLongText());
            }
        }
        return null;
    }
    ...
    protected void sanitizeAddress(Map<String, AddressComponent> components) {
        ...
        for (Map.Entry<String, AddressComponent> entry : components.entrySet()) {
            if (entry.getValue() != null) {
                String longName = sanitizeChars(entry.getValue().getLongText());
                String shortName = sanitizeChars(entry.getValue().getShortText());

                entry.getValue().setLongText(longName);
                entry.getValue().setShortText(shortName);
            }
        }
    }
    ...
    protected void transliterateComponents(Map<String, AddressComponent> components, Map<String, String> transliterationTable) {
        for (Map.Entry<String, AddressComponent> entry : components.entrySet()) {
            if (entry.getValue() != null) {
                String longName = transliterate(entry.getValue().getLongText(), transliterationTable);
                String shortName = transliterate(entry.getValue().getShortText(), transliterationTable);
